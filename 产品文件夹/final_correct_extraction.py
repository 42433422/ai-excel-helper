...
def create_final_database(customer_info, products):
    """创建最终数据库"""
    print('\n=== 创建最终数据库（I列=规格，K列=单价） ===')
    
    # 删除旧数据库
    try:
        os.remove('customer_products_final.db')
    except:
        pass
    ...
    cursor.execute('''
        CREATE TABLE products (
            ...
            产品型号 TEXT NOT NULL,
            产品名称 TEXT,
            数量_件 REAL DEFAULT 0.0,      -- 列8 (H): 数量/件
            规格_KG REAL DEFAULT 0.0,       -- 列9 (I): 规格/KG 
            数量_KG REAL DEFAULT 0.0,      -- 列10 (J): 数量/KG
            单价 REAL DEFAULT 0.0,          -- 列11 (K): 单价/元
            金额 REAL DEFAULT 0.0,          -- 列12 (L): 金额/元
            单号 TEXT,
    ...
    print(f"最终数据插入完成：{len(products)} 种产品")
...
def verify_final_database():
    ...
    cursor.execute('SELECT SUM(数量_件) FROM products')
    total_pieces = cursor.fetchone()[0] or 0
    
    cursor.execute('SELECT SUM(规格_KG) FROM products')
    ...
    print(f'\n统计汇总:')
    print(f'  产品种类: {total_count} 种')
    print(f'  总数量(件): {total_pieces:,.2f} 件')
    print(f'  总规格: {total_spec:,.2f} KG')
    print(f'  总重量: {total_quantity:,.2f} KG')
    print(f'  总单价: {total_price:,.2f} 元')
    ...
    for i, (_, p) in enumerate(products.head(5).iterrows()):
        ...
        print(f'   规格(I列): {p["规格_KG"]:.1f} KG/桶')
        print(f'   单价(K列): {p["单价"]:.2f} 元')
        print(f'   数量: {p["数量_件"]:.0f}件 / {p["数量_KG"]:.1f}KG')
        print(f'   金额: {p["金额"]:.2f} 元')
    
    conn.close()

if __name__ == "__main__":
    print("=== 修正列索引提取（I列=规格，K列=单价） ===")
    
    # 提取正确列的数据
    customer_info, products = extract_correct_columns()
    
    # 创建最终数据库
    create_final_database(customer_info, products)
    ...
    print("   K列 = 单价/元 (列11)")
    print("✅ 最终数据库已创建")
    print("文件：customer_products_final.db")