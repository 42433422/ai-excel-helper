                ...
                if '出货' in sheet_name or len(sheet_name) < 10:
                    ...
                    for index, row in df.iterrows():
                        ...
                        for j in range(min(15, len(row))):
                            ...
                            # 识别产品名称（包含特定关键词）
                            if any(keyword in cell_str for keyword in ['漆', '剂', '底', '面', '稀释', '固化', '白', '黑', '黄', '蓝', '红', '绿', '灰', '清', '哑', '亮', '分', '全', '半']):
                                if not product['产品名称']:
                            ...
                            # 识别产品型号（通常是字母数字组合，不含中文，长度适中）
                                ...
                                if not product['产品型号'] and not cell_str.isdigit():
                                    # 优先使用非纯数字的型号
                                    product['产品型号'] = cell_str
                                elif not product['产品型号']:
                                    # 如果没有非纯数字型号，再使用纯数字型号
                                    product['产品型号'] = cell_str
                        ...
                        if len(numeric_values) >= 2:
                            # 通常的顺序：数量_件、规格_KG、数量_KG、单价、金额
                            # 但不同文件可能有不同顺序，我们根据数值大小来判断
                            sorted_nums = sorted(numeric_values)
                            
                            # 规格通常在1-500之间
                            for num in numeric_values:
                                if 1 <= num <= 500 and product['规格_KG'] == 0:
                                    product['规格_KG'] = num
                                    break
                            
                            # 单价通常在5-500之间
                            for num in numeric_values:
                                if 5 <= num <= 500 and product['单价'] == 0 and num != product['规格_KG']:
                                    product['单价'] = num
                                    break
                            
                            # 数量_件通常在1-50之间
                            for num in numeric_values:
                                if 1 <= num <= 50 and product['数量_件'] == 0 and num not in [product['规格_KG'], product['单价']]:
                                    product['数量_件'] = num
                                    break
                            
                            # 金额通常是最大的数值
                            if product['金额'] == 0:
                                product['金额'] = max(numeric_values)
