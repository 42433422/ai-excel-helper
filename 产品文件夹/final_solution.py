...
# 最终解决方案：基于列名的精确提取
def extract_products_final(file_path, file_name):
    """基于列名的精确产品信息提取"""
    try:
        products = []
            ...
            try:
                ...
                if '出货' in sheet_name or len(sheet_name) < 10:
                    ...
                    if '产品名称' in df.columns:
                        print(f"  ✔️ 找到包含产品名称的工作表: {sheet_name}")
                        ...
                        for index, row in df.iterrows():
                            ...
                            if '产品名称' in row and pd.notna(row['产品名称']):
                                product['产品名称'] = str(row['产品名称']).strip()
                            
                            # 提取产品型号
                            if '产品型号' in row and pd.notna(row['产品型号']):
                                product['产品型号'] = str(row['产品型号']).strip()
                            
                            # 提取数量/件
                            if '数量/件' in row and pd.notna(row['数量/件']):
                                try:
                                    product['数量_件'] = float(row['数量/件'])
                                except:
                                    pass
                            
                            # 提取规格/KG
                            if '规格/KG' in row and pd.notna(row['规格/KG']):
                                try:
                                    product['规格_KG'] = float(row['规格/KG'])
                                except:
                                    pass
                            ...
                            if '单价' in row and pd.notna(row['单价']):
                                try:
                                    product['单价'] = float(row['单价'])
                                except:
                                    pass
                            
                            # 提取金额
                            if '金额' in row and pd.notna(row['金额']):
                                try:
                                    product['金额'] = float(row['金额'])
                                except:
                                    pass
                            
                            # 提取单号
                            if '单号' in row and pd.notna(row['单号']):
                                product['单号'] = str(row['单号']).strip()
...
def main():
    ...
    for file_path in excel_files:
        try:
            ...
            # 提取产品信息
            products = extract_products_final(file_path, file_name)
            if not products:
                print(f"  ⚠ 未找到有效产品信息")
                continue
