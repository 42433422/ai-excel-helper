...
/**
 * 产品管理页（#view-products）一级锁：用「无读头 GET /api/products/list」是否返回 403 判断服务器是否启用
 * FHD_DB_READ_TOKEN；不依赖 /api/fhd/db-tokens/status（避免该接口失败时永远不弹层）。
 *
 * 若仍无弹窗：① 须在 ProductsView 模板里挂载本组件；② 后端进程须已配置 FHD_DB_READ_TOKEN 且重启，
 * 使无 X-FHD-Db-Read-Token 的 list 请求返回 403；③ :api-base 须与调用 API 的前缀一致（跨域时用完整 origin）。
 *
 * 接入示例：
 *   <ProductsReadGate :api-base="apiBase" @unlocked="loadProductList" />
 *   所有产品/客户等受保护 GET 须合并 dbReadHeaders()（dbTokenHeaders.ts）。
 */
