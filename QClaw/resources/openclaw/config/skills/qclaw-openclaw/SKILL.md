...
> 🚨 **[MANDATORY]** 用户提到「提醒」「闹钟」「定时」「每天X点」「X分钟/小时后」「叫我」「周期」「重复」「定期」「打卡」「日报」「周报」「签到」「天气」「喝水」等涉及延时或定时的请求时，**必须创建 cron 任务**。
> 你没有内存或后台线程，口头承诺"到时候提醒"是无效的——只有创建 cron 任务才能真正注册定时执行。

#### 创建任务：**优先使用内置 cron tool**（经过 `cron-delivery-guard` 插件保护）
...
**调度字段**：
- `{"kind":"cron","expr":"0 8 * * *","tz":"Asia/Shanghai"}` — 周期 cron，必须加 `tz`
- `{"kind":"at","at":"+5m"}` 或 `{"kind":"at","at":"2026-03-26T15:00:00"}` — 一次性，相对时间无需 tz，绝对时间按系统时区填本地时间
- `{"kind":"every","everyMs":600000}` — 固定间隔（毫秒）

**`payload.message`** 是触发时 AI 收到的 prompt（不是直接发给用户的文本），需包含：`不要回复HEARTBEAT_OK，不要解释你是谁`

**delivery 规则**（🚨 关键）：

| 场景 | delivery 字段 |
|------|--------------|
| **本地会话**（webchat/terminal，sessionKey 无外部通道信号） | `{"mode":"announce"}` — **不传 channel 和 to** |
| **外部通道**（上下文有通道标签/Conversation info/agentPrompt 信号） | `{"mode":"announce","channel":"<提取>","to":"<提取>"}` — **channel 和 to 缺一不可，漏 to 消息无法送达！** |

外部通道标识提取规则：通道标签格式 `to={channel}:{rest}` → `channel` 字段取首段，`to` 字段取剩余部分。

> `cron-delivery-guard` 插件会自动注入 `bestEffort:true` 并校验 `sessionTarget`，**AI 不需要手动设置 bestEffort**。
...
# 本地会话：5分钟后提醒喝水
cron action=add job={
  "name":"喝水提醒","sessionTarget":"isolated","deleteAfterRun":true,
  "schedule":{"kind":"at","at":"+5m"},
  "payload":{"kind":"agentTurn","message":"提醒用户喝水。不要回复HEARTBEAT_OK，不要解释你是谁"},
  "delivery":{"mode":"announce"}
}

# 本地会话：每天8点打卡提醒
cron action=add job={
  "name":"每日打卡","sessionTarget":"isolated",
  "schedule":{"kind":"cron","expr":"0 8 * * *","tz":"Asia/Shanghai"},
  "payload":{"kind":"agentTurn","message":"提醒用户打卡。不要回复HEARTBEAT_OK，不要解释你是谁"},
  "delivery":{"mode":"announce"}
}

# 外部通道（如钉钉）：每天8点提醒，channel 和 to 必须同时传
cron action=add job={
  "name":"每日提醒","sessionTarget":"isolated",
  "schedule":{"kind":"cron","expr":"0 8 * * *","tz":"Asia/Shanghai"},
  "payload":{"kind":"agentTurn","message":"<prompt>"},
  "delivery":{"mode":"announce","channel":"<从上下文提取>","to":"<从上下文提取>"}
}
```
...
cron action=get id=<job_id>       # 查看详情
cron action=remove id=<job_id>    # 删除
cron action=enable id=<job_id>    # 启用
cron action=disable id=<job_id>   # 禁用
cron action=run id=<job_id>       # 立即运行（调试）
```
