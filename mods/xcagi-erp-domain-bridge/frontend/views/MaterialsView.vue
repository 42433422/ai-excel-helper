...
const erpPath = (p) => resolveErpApiPath(p)
...
const loadMaterials = async () => {
  try {
    ...
    const res = await get(erpPath('/api/materials'), params)
    if (res.success) {
...
const saveMaterial = async () => {
  ...
  try {
    const res = isEditMaterial.value
      ? await api.put(erpPath(`/api/materials/${materialForm.value.id}`), materialForm.value)
      : await post(erpPath('/api/materials'), materialForm.value)
...
const deleteMaterial = async (item) => {
  ...
  try {
    const res = await api.delete(erpPath(`/api/materials/${item.id}`))
