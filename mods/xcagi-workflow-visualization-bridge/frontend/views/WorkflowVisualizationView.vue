      ...
      <template v-else-if="docs">
        <header class="wf-viz-header">
          ...
          <div class="wf-viz-header-actions">
            <router-link :to="otherToolsLocation" class="wf-viz-back">返回员工工作流管理</router-link>
...
import { resolveOtherToolsLocation } from '@/utils/workflowNav'

function branchKindLabel(kind?: WorkflowEmployeeKind): string {
  if (kind === 'fixed_extension') return '固定扩展'
  if (kind === 'mod_extension') return '扩展工作流'
  return '内置 AI'
}

function branchKindClass(kind?: WorkflowEmployeeKind): string {
  if (kind === 'fixed_extension') return 'fixed'
  if (kind === 'mod_extension') return 'mod'
  return 'core'
}

const otherToolsLocation = resolveOtherToolsLocation()
...
.wf-viz-branch-card--mod .wf-viz-kind-badge {
  background: #ede9fe;
  color: #5b21b6;
...
.wf-viz-section--kind-mod .wf-viz-flow-head .wf-viz-kind-badge {
  background: #ede9fe;
  color: #5b21b6;
  border-color: #c4b5fd;
}

.wf-viz-section--kind-mod {
