  ...
  <teleport to="body">
    <div v-if="showOverlay" class="fhd-read-gate-root" role="dialog" aria-modal="true" aria-labelledby="fhd-global-read-title">
      ...
      <div class="fhd-read-gate-panel" @click.stop>
        <h2 id="fhd-global-read-title" class="fhd-read-gate-title">一级数据库口令（仅查看产品库）</h2>
        <p class="fhd-read-gate-desc">
          浏览产品列表、报价导出/预览等需要 <code>X-FHD-Db-Read-Token</code>（与 <code>FHD_DB_READ_TOKEN</code> 或回退策略一致）。输入后保存在本机浏览器；<strong>新增/修改/删除产品</strong>另需二级
          <code>X-FHD-Db-Write-Token</code>（<code>FHD_DB_WRITE_TOKEN</code>，见 localStorage <code>xcagi_db_write_token</code>）。
        </p>
