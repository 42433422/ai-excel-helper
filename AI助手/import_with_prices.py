...
def import_products_with_prices():
    """从Sheet2工作表导入产品和价格"""
    try:
        # 读取Sheet2工作表
        ...
        for idx, row in product_data.iterrows():
            ...
            if code and code != 'nan' and len(code) > 0:
                ...
                if code not in seen_models:
                    ...
                    valid_products.append((code, name, internal_price_val, external_price_val))
        ...
        imported_count = 0
        for code, name, internal_price, external_price in valid_products:
            # 插入产品
            cursor.execute("""
                INSERT INTO products (model_number, name, price, is_active, created_at, updated_at)
                VALUES (?, ?, ?, 1, datetime('now'), datetime('now'))
            """, (code, name, internal_price))  # 使用内部价格作为基础价格
            imported_count += 1
            logger.info(f"导入产品: {code} - {name} (内价: {internal_price}, 外价: {external_price})")
...
def recreate_customer_products_with_prices():
    """重新创建购买单位和产品的关联，使用不同的价格"""
    try:
        conn = sqlite3.connect(db_path)
        cursor = conn.cursor()
        ...
        # 获取所有活跃产品
        cursor.execute("SELECT id, model_number, name FROM products WHERE is_active = 1 ORDER BY model_number")
        ...
        for idx, row in product_data.iterrows():
            ...
            if code and code != 'nan' and len(code) > 0:
                price_map[code] = {
                    'internal': float(internal_price) if pd.notna(internal_price) and isinstance(internal_price, (int, float)) else 0.0,
                    'external': float(external_price) if pd.notna(external_price) and isinstance(external_price, (int, float)) else 0.0
                }
        ...
        for unit in units:
            unit_id = unit[0]
            unit_name = unit[1]
            logger.info(f"为购买单位 {unit_name} (ID: {unit_id}) 创建关联")
            ...
            for product in products:
                ...
                if '1' in unit_name:  # 蕊芯家私1 使用内价
                    custom_price = price_map.get(model_number, {}).get('internal', 0.0)
                    logger.debug(f"  - 使用内价: {model_number} = {custom_price}")
                else:  # 蕊芯家私 使用外价
                    custom_price = price_map.get(model_number, {}).get('external', 0.0)
                    logger.debug(f"  - 使用外价: {model_number} = {custom_price}")
                
                # 插入关联
                cursor.execute("""
                    INSERT INTO customer_products (unit_id, product_id, custom_price, is_active, created_at, updated_at)
                    VALUES (?, ?, ?, 1, datetime('now'), datetime('now'))
                """, (unit_id, product_id, custom_price))
                association_count += 1
...
def verify_import():
    ...
    try:
        ...
        # 检查几个产品的价格设置
        cursor.execute("""
            SELECT p.model_number, p.name, pu.unit_name, cp.custom_price
            FROM customer_products cp
            JOIN products p ON cp.product_id = p.id
            JOIN purchase_units pu ON cp.unit_id = pu.id
            WHERE cp.is_active = 1
            ORDER BY p.model_number, pu.unit_name
            LIMIT 10
        """)
        
        price_samples = cursor.fetchall()
        logger.info("产品价格设置示例 (前10个):")
        for model, name, unit_name, price in price_samples:
            logger.info(f"  - {model} ({name}) - {unit_name}: {price}")
...
def main():
    """主函数"""
    print("=== 根据价格结构导入产品和价格 ===")
    print("蕊芯家私1使用内价，蕊芯家私使用外价")
    
    # 清理数据库
    if clean_database():
        # 导入产品和基础价格
        imported_count = import_products_with_prices()
        if imported_count > 0:
            # 重新创建关联，使用不同的价格
            association_count = recreate_customer_products_with_prices()
