...
def create_ruixin_units():
    ...
    try:
        ...
        created_units = []
        for unit_name, contact, phone, discount, notes in units:
            cursor.execute("""
            ...
            created_units.append({'id': unit_id, 'name': unit_name, 'type': notes})
            print(f"✅ 创建购买单位: {unit_name} (ID: {unit_id}, 类型: {notes})")
...
def import_ruixin_products(products, units):
    ...
    try:
        # 获取购买单位ID
        ruixin_outer = next((u for u in units if u['type'] == '外'), None)
        ruixin_inner = next((u for u in units if u['type'] == '内'), None)
...
def verify_import():
    ...
    try:
        # 检查购买单位
        cursor.execute("""
            SELECT id, unit_name, contact_person, notes 
            FROM purchase_units 
        ...
        print(f"📋 蕊芯家私购买单位:")
        for unit in units:
            print(f"  ID: {unit[0]}, 名称: {unit[1]}, 联系人: {unit[2]}, 类型: {unit[3]}")
