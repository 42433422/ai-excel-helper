...
def search_products(keyword):
    with get_db() as db:
        return db.query(Product).filter(Product.name.like(f"%{keyword}%")).limit(20).all()


def search_products_by_model(model_number):
    with get_db() as db:
        return (
            db.query(Product).filter(Product.model_number.like(f"%{model_number}%")).limit(20).all()
        )
...
def search_customers(keyword):
    with get_db() as db:
        return (
            ...
            .filter(PurchaseUnit.unit_name.like(f"%{keyword}%"), PurchaseUnit.is_active == True)
            .limit(20)
            .all()
        )
...
def query_recent_shipments(days=7, limit=20):
    ...
    with get_db() as db:
        ...
        return (
            ...
            .filter(ShipmentRecord.created_at >= start_date)
...
def query_product_price(product_name):
    with get_db() as db:
        product = db.query(Product).filter(Product.name.like(f"%{product_name}%")).first()
...
def search_materials(keyword):
    with get_db() as db:
        return db.query(Material).filter(Material.name.like(f"%{keyword}%")).limit(20).all()
