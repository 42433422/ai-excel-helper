...
def extract_text_with_ocr(image_path: str, use_regions: bool = True) -> Dict[str, Any]:
    ...
    try:
        ...
        # 4. 基于网格布局智能配对字段
        fields = _pair_fields_by_grid(text_blocks, horizontal_lines, vertical_lines)
        
        logger.info(f"识别到 {len(fields)} 个字段")
        for i, field in enumerate(fields):
            logger.info(f"字段 {i+1}: {field['label']}: {field['value']} (类型：{field['type']})")
...
def _pair_fields_by_grid(text_blocks: List[Dict], horizontal_lines: List[int], vertical_lines: List[int]) -> List[Dict[str, Any]]:
    """
    基于网格布局智能配对字段（标签 + 值）
    ...
    # 3. 同一行内，按 X 坐标排序（从左到右）并配对
    fields = []
    for group in row_groups:
        # 同一行内按 X 坐标排序
        group_sorted = sorted(group, key=lambda x: x['left'])
