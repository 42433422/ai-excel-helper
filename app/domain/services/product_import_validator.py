...
class ProductImportValidator:
    ...
    def _validate_single_product(
        self, product: dict[str, Any], index: int, result: ValidationResult
    ):
        """验证单个产品数据"""
        prefix = f"产品[{index}]"
        ...
        if len(name) > self._max_name_length:
            result.add_warning(
                f"{prefix}.name", f"产品名称过长 ({len(name)} > {self._max_name_length})"
            )
        ...
        if "price" in product:
            ...
            if not isinstance(price, (int, float)):
                ...
                result.add_warning(f"{prefix}.price", f"价格超出正常范围 (>{self._max_price})")
    ...
    def validate_batch_size(
        self, products: list[dict[str, Any]], max_size: int = 1000
    ) -> ValidationResult:
        """验证批次大小"""
        result = ValidationResult()

        if len(products) > max_size:
            result.add_error("batch_size", f"批次大小超出限制 ({len(products)} > {max_size})")
    ...
    def suggest_fixes(self, result: ValidationResult) -> list[dict[str, Any]]:
        ...
        for error in result.errors:
            if error.field.endswith(".name") and "为空" in error.message:
                suggestions.append({"field": error.field, "suggestion": "请提供产品名称"})
            elif error.field.endswith(".price"):
                suggestions.append({"field": error.field, "suggestion": "请提供有效的价格数字"})
